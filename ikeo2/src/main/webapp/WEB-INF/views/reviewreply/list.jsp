...
$(function(){
	...
	$(".replyDeleteBtn").click(function(){
		...
		location = "/reviewreply/delete.do?rno=" + rno + "&no=" + no;
...
<div class="container" id="replyDiv">
	...
	<c:if test="${!empty login}">
		<div class="well" style="background: #dcdcdc;">
			<form action="/reviewreply/write.do" method="post">
				<input type="hidden" name="no" value="${vo.reNo }" id="no">
	...
	<c:if test="${not empty replyPageObject}">
		<c:forEach items="${replyPageObject }" var="ReviewReplyVO">
			<div class="panel panel-default dataRow">
				<div class="panel-heading">
					답변 번호 : <span class="rno">${ReviewReplyVO.rno }</span>
					<span class="pull-right">${ReviewReplyVO.writeDate }</span>
				</div>
				<div class="panel-body content">${ReviewReplyVO.content }</div>
				<div class="panel-footer">
					${ReviewReplyVO.name }(${ReviewReplyVO.id })
					<c:if test="${!empty login && ReviewReplyVO.id == login.id && login.grade_no == 9}">
...
<div id="updateModal" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
    	<form action="/reviewreply/update.do" method="post">
    	
    		<input type="hidden"  name="no" value="${vo.reNo }">
    		<input type="hidden" name="query" value="<%= request.getQueryString() %>">
