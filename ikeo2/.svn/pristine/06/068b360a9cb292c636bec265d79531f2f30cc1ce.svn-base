package com.ikeo.reviewreply.controller;
...
public class ReviewReplyController {
	...
	public String write(ReviewReplyVO vo, HttpSession session, RedirectAttributes rttr) throws Exception {

		vo.setId(((LoginVO) session.getAttribute("login")).getId());
		System.out.println("ReviewReplyController.write().vo - " + vo);
		service.write(vo);
		...
		return "redirect:/review/view.do?no=" + vo.getNo();
	}
	
	
	@PostMapping("/update.do")
	public String update(ReviewReplyVO vo,Long no,RedirectAttributes rttr,Model model, HttpSession session)
			throws Exception {

		System.out.println("ReviewReplyController.update().vo - " + vo);
		vo.setId(((LoginVO) session.getAttribute("login")).getId());
	...
	public String delete(ReviewReplyVO vo, Long no, HttpSession session, RedirectAttributes rttr) throws Exception {
		
		System.out.println("ReviewReplyController.delete().no - " + no);
		vo.setId(((LoginVO) session.getAttribute("login")).getId());
		service.delete(vo);
