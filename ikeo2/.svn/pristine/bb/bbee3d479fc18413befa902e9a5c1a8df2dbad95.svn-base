           ...
           updateButton.click(function () {
               ...
               $.ajax({
                   ...
                   success: function (data) {
                       ...
                       panel.find('.button-group').show(); // 수정 버튼 및 삭제 버튼 다시 표시
                       location.reload();
                   },
                   error: function () {
                       alert("댓글수정 실패");
    ...
    // 대댓글 달기 이벤트 처리
       $(".replyAnswerBtn").click(function () {
           ...
           replyForm.find("#cancelBtn").click(function () {
               ...
               replyForm.remove();
               dataRow.find('.button-group').show(); // 수정 버튼 및 삭제 버튼 다시 표시
           });

           // 기존의 입력 폼 및 답글 버튼 숨기기
           dataRow.find('.replyForm').remove();
           dataRow.find('.button-group').hide();
...
<div id="replyDiv" style="margin-left:15px;">
	<h3 style="font-weight: 700; font-size: 20px;">댓글 <span style="color: #35C5F0; font-weight: 700;">${fn:length(replyList)}</span></h3>
					...
					<div class="text-right button-group">
						<c:if test="${!empty login && login.id == replyVO.reply_id}">
							<button type="button"
								...
								<c:if test="${(!empty login && login.id == replyVO.reply_id) || login.grade_no >= 8 }">
							<button type="button"
	...
	<div>
		<replayPageNav:replayPageNav listURI="view.do" pageObject="${replyPageObject}" 
		query="${query}"></replayPageNav:replayPageNav>
	</div>
</div>